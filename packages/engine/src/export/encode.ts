...
import type { Project, Rate, Time } from "@videola/core";
...
async function writeVideo(request: ExportRequest, pass: VideoPass): Promise<void> {
  ...
  for (const frame of request.frames) {
    ...
    pass.compositor.render(request.project, frame.at, pictures);
...
async function gather(
  ...
  for (const item of drawList(project, frame.at).items) {
