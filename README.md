...
**Installable and offline.** The browser build carries a manifest and a service worker: it installs
as an application, opens without a network, and notices a new build — a new worker waits, the editor
offers a reload, and nothing is swapped under a session with unsaved work in it.

**Phone, tablet and desktop.** Not a second implementation — the same code, with the panels taking
turns behind a tab bar where there is no room for them side by side. The layout is detected from the
...
core is up, and offering an update in the editor's own dialogue with a real progress bar — building
Windows, Linux and macOS installers with an opt-in signed
...
as separating a voice from noise sharing its band. The magnetic timeline is deliberately absent: the
