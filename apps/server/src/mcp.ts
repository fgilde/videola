  ...
  const server = new Server(
    { name: "videola", version: APP_VERSION },
    {
      ...
      instructions:
        ...
        "Read project_describe after a change instead of guessing what landed, and project_save " +
        "to write a .videola archive. There is no frame renderer on the server, so no tool can " +
        "show you a picture.",
    },
  );
...
function optionalRevision(value: unknown): number | undefined {
  ...
  if (typeof value !== "number" || !Number.isInteger(value)) {
    throw new Error("ifRevision must be an integer");
  }
  return value;
}
